...
class RecommendationEngine:
    ...
    async def generate_recommendations(
        ...
        typosquat_findings = [f for f in quality_findings if "typosquat" in f.get("details", {}).get("risk_type", "").lower()]
    ...
    def _analyze_direct_dependencies(
        ...
        for component, component_vulns in vulns_by_component.items():
            ...
            if unreachable_count > 0 and unreachable_count == len(component_vulns):
                desc_parts.append(f"All vulnerabilities are unreachable - lower priority.")
            ...
            recommendations.append(
                Recommendation(
                    ...
                    action={
                        ...
                        "high_epss_cves": [
                            v.cve_id for v in component_vulns 
    ...
    def _recommendation_score(self, rec: Recommendation) -> int:
        ...
        if reachable_count > 0:
            ...
            threat_intel_score += (reachable_count - reachable_critical - reachable_high) * 25
    ...
    def _detect_critical_hotspots(
        ...
        package_findings: Dict[str, Dict[str, Any]] = defaultdict(lambda: {
            "vulnerabilities": [],
            "quality_issues": [],
            "license_issues": [],
            "malware": [],
        ...
        for f in findings:
            ...
            if finding_type == "vulnerability":
                ...
                pkg_data["total_risk_score"] += details.get("risk_score", 0) or self.severity_weights.get(severity, 0)
        ...
        for pkg_name, pkg_data in package_findings.items():
            ...
            if pkg_data["high_epss_count"] > 0 and pkg_data["reachable_count"] > 0:
                is_hotspot = True
                hotspot_reasons.append(f"{pkg_data['high_epss_count']} high-EPSS CVE(s), {pkg_data['reachable_count']} reachable")
            
            if vuln_count >= 3 and critical_high >= 1:
                is_hotspot = True
                hotspot_reasons.append(f"{vuln_count} vulnerabilities ({pkg_data['critical_count']} critical, {pkg_data['high_count']} high)")
        ...
        for hotspot in hotspots[:10]:  # Top 10 hotspots
            priority = Priority.CRITICAL if (hotspot["has_malware"] or hotspot["kev_count"] > 0 or hotspot["critical_count"] > 0) else Priority.HIGH
    ...
    def _detect_known_exploits(
        ...
        if ransomware_vulns:
            ...
            cves = list(set(f.get("finding_id", "") for f in ransomware_vulns if f.get("finding_id", "").startswith("CVE-")))
        ...
        if kev_vulns:
            ...
            cves = list(set(f.get("finding_id", "") for f in kev_vulns if f.get("finding_id", "").startswith("CVE-")))
            ...
            recommendations.append(
                Recommendation(
                    ...
                    impact={
                        "critical": len([v for v in kev_vulns if v.get("severity") == "CRITICAL"]),
                        "high": len([v for v in kev_vulns if v.get("severity") == "HIGH"]),
                        "medium": len([v for v in kev_vulns if v.get("severity") == "MEDIUM"]),
                        "low": 0,
                        "total": len(kev_vulns),
        ...
        if high_epss_vulns:
            ...
            cves = list(set(f.get("finding_id", "") for f in high_epss_vulns if f.get("finding_id", "").startswith("CVE-")))
            
            # Get max EPSS score
            max_epss = max(f.get("details", {}).get("epss_score", 0) for f in high_epss_vulns)
            ...
            recommendations.append(
                Recommendation(
                    ...
                    impact={
                        "critical": len([v for v in high_epss_vulns if v.get("severity") == "CRITICAL"]),
                        "high": len([v for v in high_epss_vulns if v.get("severity") == "HIGH"]),
                        "medium": len([v for v in high_epss_vulns if v.get("severity") == "MEDIUM"]),
                        "low": 0,
        ...
        return [
            Recommendation(
                ...
                impact={
                    ...
                    "high": high_count,
                    "medium": len([f for f in eol_findings if f.get("severity") == "MEDIUM"]),
    ...
    def _identify_quick_wins(
        ...
        for qw in quick_wins[:5]:  # Top 5 quick wins
            ...
            recommendations.append(
                Recommendation(
                    type=RecommendationType.SINGLE_UPDATE_MULTI_FIX if qw["vuln_count"] >= 3 else RecommendationType.QUICK_WIN,
    ...
    def _detect_toxic_dependencies(
        ...
        package_risks: Dict[str, Dict[str, Any]] = defaultdict(lambda: {
            "risk_factors": [],
            "total_score": 0,
            "vulns": [],
            "details": {},
            ...
            recommendations.append(
                Recommendation(
                    ...
                    impact={
                        "critical": len([v for v in pkg["vulns"] if v.get("severity") == "CRITICAL"]),
                        "high": len([v for v in pkg["vulns"] if v.get("severity") == "HIGH"]),
                        "medium": len([v for v in pkg["vulns"] if v.get("severity") == "MEDIUM"]),
                        "low": 0,
                        "total": len(pkg["vulns"]),
        ...
        if transitive_with_vulns:
            ...
            recommendations.append(
                Recommendation(
                    ...
                    affected_components=[f"{t['name']}@{t['version']} (via {t['parent']})" for t in transitive_with_vulns[:10]],
                    action={
