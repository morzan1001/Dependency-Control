...
class MaintainerRiskAnalyzer(Analyzer):
    ...
    async def _check_component(self, client: httpx.AsyncClient, component: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        overall_severity = "CRITICAL" if max_severity >= 4 else "HIGH" if max_severity >= 3 else "MEDIUM" if max_severity >= 2 else "LOW"
    ...
    async def _check_pypi(self, client: httpx.AsyncClient, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "days_since_release": (datetime.now(timezone.utc) - latest_release_date).days if latest_release_date else None,
    ...
    async def _check_npm(self, client: httpx.AsyncClient, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if time_info.get("modified"):
                try:
                    latest_release_date = datetime.fromisoformat(time_info["modified"].replace("Z", "+00:00"))
                except:
                    pass
    ...
    async def _check_github(self, client: httpx.AsyncClient, repo: str) -> Optional[Dict[str, Any]]:
