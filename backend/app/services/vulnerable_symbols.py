...
SYMBOL_PATTERNS = [
    ...
    re.compile(r'(?:the\s+)?(?:function|method|procedure)\s+[`\'"]?(\w+(?:\.\w+)*)[`\'"]?', re.IGNORECASE),
    
    # "`functionName` function" or "`methodName()` method"
    re.compile(r'[`\'"](\w+(?:\.\w+)*)\(\)?[`\'"]?\s+(?:function|method)', re.IGNORECASE),
    
    # "via `functionName`" or "through `methodName`"
    re.compile(r'(?:via|through|using|in)\s+[`\'"](\w+(?:\.\w+)*)\(\)?[`\'"]?', re.IGNORECASE),
    
    # "calling `functionName`" or "invoking `methodName`"
    re.compile(r'(?:calling|invoking|executing)\s+[`\'"]?(\w+(?:\.\w+)*)\(\)?[`\'"]?', re.IGNORECASE),
    
    # "vulnerable `functionName`" or "affected `methodName`"
    re.compile(r'(?:vulnerable|affected|insecure)\s+[`\'"]?(\w+(?:\.\w+)*)[`\'"]?', re.IGNORECASE),
...
FALSE_POSITIVE_SYMBOLS = {
    ...
    "the", "a", "an", "this", "that", "which", "where", "when",
    "http", "https", "www", "com", "org", "net", "io",
    "api", "url", "uri", "json", "xml", "html", "css",
    "get", "set", "put", "post", "delete", "patch",  # HTTP methods (context-dependent)
    "cve", "cwe", "ghsa", "nvd",
    "version", "versions", "update", "updates",
    "fix", "fixed", "patch", "patched",
    "null", "none", "true", "false", "undefined",
    # Too generic
    "object", "array", "string", "number", "boolean",
    "file", "path", "data", "value", "key", "name",
}
...
KNOWN_PATTERNS = {
    "prototype_pollution": [
        ...
        "set", "setWith", "zipObjectDeep", "mergeWith",
    ],
    "template_injection": [
        "template", "render", "compile", "eval",
    ],
    "command_injection": [
        "exec", "spawn", "execSync", "spawnSync", "shell",
        "system", "popen", "subprocess", "run",
    ],
    "sql_injection": [
        "query", "execute", "raw", "rawQuery",
    ],
    "path_traversal": [
        "readFile", "writeFile", "open", "path", "join",
    ],
    "deserialization": [
        "deserialize", "unserialize", "unpickle", "loads", "load",
        "readObject", "ObjectInputStream",
    ],
    "xxe": [
        "parse", "parseXML", "XMLParser", "DocumentBuilder",
    ],
    "jndi": [
        "lookup", "JndiLookup", "JndiManager", "InitialContext",
    ],
}
...
def extract_symbols_from_text(
    ...
    for pattern in SYMBOL_PATTERNS:
        ...
        for match in matches:
            ...
            symbol = match.strip().lower() if isinstance(match, str) else str(match).strip().lower()
...
def _detect_vulnerability_type(text: str) -> Optional[str]:
    ...
    if "command injection" in text_lower or "remote code execution" in text_lower or "rce" in text_lower:
