...
async def enrich_findings_with_reachability(
    ...
    if not scan_id:
        logger.warning(f"No scan_id available for reachability enrichment")
        return 0
...
def _analyze_reachability(
    ...
    # =========================================
    # Level 2: Symbol-based Analysis (Heuristic)
    # =========================================
    ...
    if not extracted.symbols:
        ...
        result["confidence_score"] = 0.5  # Medium - package is imported but unknown functions
        result["analysis_level"] = "import"
        result["message"] = f"Package is imported in {import_count} file(s). Could not determine specific vulnerable functions."
        return result
