...
class MattermostProvider(NotificationProvider):
    ...
    async def _get_user_id_by_username(self, client: httpx.AsyncClient, username: str, base_url: str, headers: dict) -> Optional[str]:
        # Remove @ if present
    ...
    async def _create_dm_channel(self, client: httpx.AsyncClient, user_id: str, base_url: str, headers: dict) -> Optional[str]:
    ...
    async def _get_channel_id_by_name(self, client: httpx.AsyncClient, channel_name: str, base_url: str, headers: dict) -> Optional[str]:
        # Remove # if present
        ...
        try:
            ...
            for team in teams:
                ...
                chan_response = await client.get(f"{base_url}/api/v4/teams/{team_id}/channels/name/{channel_name}", headers=headers)
    ...
    async def send(self, destination: str, subject: str, message: str, system_settings: Optional[SystemSettings] = None) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                elif not any(c in destination for c in ["-", " "]) or destination.startswith("#"):
