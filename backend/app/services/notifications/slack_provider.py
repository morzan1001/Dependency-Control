...
class SlackProvider(NotificationProvider):
    async def _refresh_token(self, system_settings: SystemSettings) -> Optional[str]:
        ...
        """
        if not system_settings.slack_client_id or not system_settings.slack_client_secret or not system_settings.slack_refresh_token:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    logger.error(f"Failed to refresh Slack token. Status: {response.status_code}, Body: {response.text}")
                    return None
    ...
    async def send(self, destination: str, subject: str, message: str, system_settings: Optional[SystemSettings] = None) -> bool:
        ...
        # Buffer of 5 minutes
        if system_settings.slack_token_expires_at and system_settings.slack_token_expires_at < (time.time() + 300):
            ...
            if new_token:
                ...
                logger.warning("Failed to refresh Slack token, attempting to use existing token.")
