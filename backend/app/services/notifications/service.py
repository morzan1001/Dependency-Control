...
class NotificationService:
    ...
    async def _send_based_on_prefs(self, user: User, prefs: Dict[str, List[str]], event_type: str, subject: str, message: str, system_settings: Optional[SystemSettings] = None):
        channels = prefs.get(event_type, [])
        
        if "email" in channels and user.email:
            await self.email_provider.send(user.email, subject, message, system_settings=system_settings)
        
        if "slack" in channels and user.slack_username:
            await self.slack_provider.send(user.slack_username, subject, message, system_settings=system_settings)
            
        if "mattermost" in channels and user.mattermost_username:
            await self.mattermost_provider.send(user.mattermost_username, subject, message, system_settings=system_settings)

    async def notify_user(self, user: User, event_type: str, subject: str, message: str, db=None):
        """
        ...
        await self._send_based_on_prefs(user, user.notification_preferences, event_type, subject, message, system_settings)

    async def notify_users(self, users: List[User], event_type: str, subject: str, message: str, db=None):
        """
