...
class ThreatIntelligenceStats(BaseModel):
    ...
    kev_ransomware_count: int = Field(
        0, description="Count of KEV findings with known ransomware use"
    )
    ...
    active_exploitation_count: int = Field(
        0, description="Count of actively exploited vulnerabilities"
    )
...
class ReachabilityStats(BaseModel):
    ...
    analyzed_count: int = Field(
        0, description="Count of vulnerabilities analyzed for reachability"
    )
    reachable_count: int = Field(
        0, description="Count of confirmed reachable vulnerabilities"
    )
    likely_reachable_count: int = Field(
        0, description="Count of likely reachable vulnerabilities"
    )
    unreachable_count: int = Field(
        0, description="Count of unreachable vulnerabilities"
    )
    unknown_count: int = Field(
        0, description="Count of vulnerabilities with unknown reachability"
    )
...
class PrioritizedCounts(BaseModel):
    ...
    actionable_critical: int = Field(
        0, description="Critical vulns that are exploitable AND reachable"
    )
    actionable_high: int = Field(
        0, description="High vulns that are exploitable AND reachable"
    )
    actionable_total: int = Field(
        0, description="Total actionable vulns (KEV/high-EPSS AND reachable)"
    )

    # Deprioritized counts (vulns you can safely defer)
    deprioritized_count: int = Field(
        0, description="Vulns that are unreachable or low-EPSS without KEV"
    )
...
class Stats(BaseModel):
    ...
    adjusted_risk_score: float = Field(
        0.0, description="Risk score adjusted for exploitability and reachability"
    )

    # Threat intelligence stats
    threat_intel: Optional[ThreatIntelligenceStats] = Field(
        None, description="EPSS/KEV statistics"
    )

    # Reachability stats
    reachability: Optional[ReachabilityStats] = Field(
        None, description="Reachability analysis statistics"
    )

    # Prioritized counts
    prioritized: Optional[PrioritizedCounts] = Field(
        None, description="Intelligently prioritized vulnerability counts"
    )
