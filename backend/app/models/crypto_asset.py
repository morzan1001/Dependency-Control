...
class CryptoAsset(BaseModel):
    ...
    asset_type: CryptoAssetType = Field(..., description="Type of crypto asset: algorithm, certificate, protocol, or related-crypto-material")

    # Algorithm-only
    primitive: Optional[CryptoPrimitive] = Field(None, description="Cryptographic primitive classification for algorithm assets (hash, block-cipher, pke, etc.)")
    variant: Optional[str] = Field(None, description="Algorithm variant/instance (e.g. 'RSA-OAEP', 'AES-256-GCM')")
    parameter_set_identifier: Optional[str] = Field(None, description="Parameter set identifier from CycloneDX (often key size as string)")
    ...
    signature_algorithm_ref: Optional[str] = Field(None, description="bom-ref of the algorithm used to sign this certificate")
    certificate_format: Optional[str] = Field(None, description="Certificate format identifier (e.g. X.509)")

    # Protocol-only
    protocol_type: Optional[str] = Field(None, description="Protocol identifier (e.g. tls, ssh, ipsec) for protocol assets")
    version: Optional[str] = Field(None, description="Protocol version string (e.g. '1.2', '1.3')")
    cipher_suites: List[str] = Field(default_factory=list, description="Cipher suites advertised/negotiated by a protocol asset")

    # Context
    occurrence_locations: List[str] = Field(default_factory=list, description="Source locations (file paths, binary offsets) where this asset was detected")
    ...
    related_dependency_purls: List[str] = Field(default_factory=list, description="PURLs of software components linked to this crypto asset")

    properties: Dict[str, str] = Field(default_factory=dict, description="Passthrough of additional CycloneDX properties")

    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Persistence timestamp (UTC)")
