...
async def slack_callback(
    ...
    if not system_settings.slack_client_id or not system_settings.slack_client_secret:
        raise HTTPException(status_code=400, detail="Slack Client ID and Client Secret must be configured in System Settings")
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
             raise HTTPException(status_code=400, detail="Failed to exchange code with Slack")
