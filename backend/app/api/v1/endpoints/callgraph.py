...
from app.models.callgraph import Callgraph, ImportEntry, CallEdge, ModuleUsage
...
def parse_madge_format(data: Dict[str, Any], language: str) -> tuple[List[ImportEntry], List[CallEdge], Dict[str, ModuleUsage]]:
    """
...
def parse_pyan_format(data: Dict[str, Any], language: str) -> tuple[List[ImportEntry], List[CallEdge], Dict[str, ModuleUsage]]:
    """
...
def parse_generic_format(data: Dict[str, Any], language: str) -> tuple[List[ImportEntry], List[CallEdge], Dict[str, ModuleUsage]]:
    """
...
async def upload_callgraph(
    ...
    if not scan_id and request.pipeline_id:
        ...
        if existing_scan:
            ...
            logger.debug(f"Resolved scan_id {scan_id} from pipeline_id {request.pipeline_id}")
