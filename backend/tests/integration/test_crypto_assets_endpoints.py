...
async def test_list_filters_by_asset_type(db):
    ...
    repo = CryptoAssetRepository(db)
    items = await repo.list_by_scan("proj2", "sc", limit=100,
...
async def test_list_filters_by_primitive(db):
    ...
    await CryptoAssetRepository(db).bulk_upsert("proj3", "sc", [
        CryptoAsset(project_id="proj3", scan_id="sc", bom_ref="a",
                    name="AES", asset_type=CryptoAssetType.ALGORITHM,
                    primitive=CryptoPrimitive.BLOCK_CIPHER),
        CryptoAsset(project_id="proj3", scan_id="sc", bom_ref="b",
                    name="MD5", asset_type=CryptoAssetType.ALGORITHM,
                    primitive=CryptoPrimitive.HASH),
    ])

    repo = CryptoAssetRepository(db)
    items = await repo.list_by_scan("proj3", "sc", limit=100,
                                    primitive=CryptoPrimitive.BLOCK_CIPHER)
...
async def test_list_filters_by_name_search(db):
    ...
    await CryptoAssetRepository(db).bulk_upsert("proj4", "sc", [
        CryptoAsset(project_id="proj4", scan_id="sc", bom_ref="a",
                    name="AES-256-GCM", asset_type=CryptoAssetType.ALGORITHM),
        CryptoAsset(project_id="proj4", scan_id="sc", bom_ref="b",
                    name="RSA-2048", asset_type=CryptoAssetType.ALGORITHM),
    ])

    repo = CryptoAssetRepository(db)
    items = await repo.list_by_scan("proj4", "sc", limit=100,
                                    name_search="AES")
...
async def test_get_single_crypto_asset(db):
    ...
    asset = CryptoAsset(
        project_id="proj5", scan_id="sc", bom_ref="x",
        name="AES", asset_type=CryptoAssetType.ALGORITHM,
...
async def test_summary_endpoint(db):
    ...
    await CryptoAssetRepository(db).bulk_upsert("proj6", "sc", [
        CryptoAsset(project_id="proj6", scan_id="sc", bom_ref="a",
                    name="AES", asset_type=CryptoAssetType.ALGORITHM),
        CryptoAsset(project_id="proj6", scan_id="sc", bom_ref="b",
                    name="cert1", asset_type=CryptoAssetType.CERTIFICATE),
        CryptoAsset(project_id="proj6", scan_id="sc", bom_ref="c",
                    name="cert2", asset_type=CryptoAssetType.CERTIFICATE),
    ])
