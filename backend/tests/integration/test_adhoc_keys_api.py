"""CRUD for ad-hoc analysis API keys."""
...
async def _key_count(db):
    return await db["adhoc_api_keys"].count_documents({})
...
async def test_a_key_is_visible_and_revocable_only_to_its_owner(client, db):
    owner_headers = _headers([Permissions.ANALYZE_ADHOC])
    stranger_headers = _headers([Permissions.ANALYZE_ADHOC], subject=_OTHER_OWNER)
...
@pytest.mark.asyncio
async def test_missing_permission_is_403(client, db):
    headers = _headers([Permissions.PROJECT_READ])
