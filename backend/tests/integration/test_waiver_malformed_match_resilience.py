...
def _finding_doc(scan_id, fid, anchor, ch, line, rule="OPENGREP:r", file="a.py", match=None):
    if match is None:
        match = MatchSignature(
            rule_key=rule, file_key=file, anchor=anchor,
            anchor_kind="scanner_fp", content_hash=ch, last_line=line,
        ).model_dump()
    return {
        "_id": fid, "scan_id": scan_id, "finding_id": fid, "type": "sast", "component": file,
        "severity": "HIGH", "description": "d", "waived": False, "match": match,
    }
...
async def test_malformed_waiver_match_dict_is_skipped_others_applied():
    ...
    w_good = _Waiver(
        "w_good", "false_positive",
        MatchSignature(rule_key="OPENGREP:r", file_key="a.py", anchor="fpB",
                       anchor_kind="scanner_fp", content_hash="c2", last_line=20),
    )
